...
public class PaymentService {
    ...
    private final AsyncProcessorService asyncService;
    ...
    public PaymentService(MerchantRepository merchants, PaymentRepository payments, 
                          ...
                          PaymentStrategyFactory strategyFactory, AsyncProcessorService asyncService) {
    ...
    public PaymentResponse createPayment(String auth, String idemKey, PaymentRequest req) {
        ...
        PaymentStrategy strategy = strategyFactory.getStrategy(req.method());
        BigDecimal total = strategy.calculateTotal(req.amount(), req.installments());
        Double interestRate = "CARD".equalsIgnoreCase(req.method()) && req.installments() > 1 ? 1.0 : null;
        ...
        asyncService.submit(() -> processAndWebhook(payment.getId()));
    ...
    private void processAndWebhook(String paymentId) {
        System.out.println("[Thread-" + Thread.currentThread().threadId() + "] Processando pagamento: " + paymentId);
        try { Thread.sleep(delay); } catch (InterruptedException ignored) {}
        
        var pOpt = payments.findById(paymentId);
    ...
    private void sendWebhook(Payment p) {
        ...
        asyncService.submit(() -> tryDeliver(delivery.getId()));
    }

    private void tryDeliver(Long deliveryId) {
        var dOpt = deliveries.findById(deliveryId);
        if (dOpt.isEmpty()) return;
        var d = dOpt.get();
        
        System.out.println("[Thread-" + Thread.currentThread().threadId() + "] Tentando webhook ID: " + deliveryId);
